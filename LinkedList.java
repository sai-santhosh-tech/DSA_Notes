...
10) Odd Even Linked List
	...
	// The first node is considered odd, and the second node is even, and so on.[Even Odd Nodes Problem]
  ...
  // Segregate Evens and Odds in a Linked List[ Based on value inside LinkedList]
...
class Solution {
    Node divide(Node head) {
       ...
       if(even != null) even.next = oddStart; // To avoid null pointer, If list contains only odd numbers
       
       if(odd != null) odd.next = null; // Without this, the last odd node may still point to an old node from the original linked list, creating incorrect links or cycles. 
       
       
       if(evenStart == null){ // If list contains only odd numbers
           evenStart = oddStart;
       }
...
Add Numbers Represented by Linked Lists(GFG)
Given two linked lists where each node contains a single digit, add the two numbers and return the result as a linked list.
Example
Input:
List 1: 1 -> 2 -> 3
List 2: 9 -> 9 -> 9
...
Approach => Since digits are stored in forward order:
a) Reverse both linked lists
b) Add digits one by one with carry
c) Reverse the result list
...
class Solution {
    public Node addTwoLists(Node head1, Node head2) {
        ...
        Node list1 = reverse(head1);
        Node list2 = reverse(head2);
        ...
        while(list1 != null || list2 != null || carry > 0){
            ...
            int sum = carry;
            sum = ((list1 != null) ? list1.data : 0) + ((list2 != null) ? list2.data : 0) + sum;
            int digit = sum % 10;
            carry = sum/10;
